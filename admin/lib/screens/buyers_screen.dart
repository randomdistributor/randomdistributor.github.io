...
class _BuyersScreenState extends State<BuyersScreen> {
  ...
  Future<void> _add() async {
    final ok = await showPartyCreateDialog(context, role: 'buyer');
    if (ok == true) _reload();
  }

  Future<void> _edit(Map<String, dynamic> row) async {
    final ok = await showPartyEditDialog(context, role: 'buyer', row: row);
    if (ok == true) _reload();
  }
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          if (rows.isEmpty) {
            return const Center(child: Text('No buyers yet. Add one.'));
          }
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return Card(
                child: ListTile(
                  title: Text(r['name'] ?? ''),
                  subtitle: Text(r['business_name'] ?? r['address'] ?? ''),
